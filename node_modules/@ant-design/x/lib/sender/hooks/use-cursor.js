...
const useCursor = options => {
  ...
  const getRange = (0, _react.useCallback)(() => {
  ...
  const setRange = (0, _react.useCallback)((range, selection) => {
    if (!range || !selection) {
      return;
    }
    try {
      ...
      (0, _warning.default)(false, 'Sender', `Failed to set range: ${error}`);
    }
  }, []);
  const removeAllRanges = (0, _react.useCallback)(() => {
    const selection = getSelection();
    if (!selection) {
      return;
    }
    try {
      ...
      (0, _warning.default)(false, 'Sender', `Failed to remove all ranges: ${error}`);
    }
  }, [getSelection]);
  const focus = (0, _react.useCallback)((targetNode, preventScroll = false) => {
    ...
    try {
      ...
      (0, _warning.default)(false, 'Sender', `Failed to focus element: ${error}`);
    }
  }, []);
  const setEndCursor = (0, _react.useCallback)((targetNode, preventScroll) => {
    if (!targetNode) {
      return;
    }
    ...
    if (range && selection) {
      try {
        ...
        (0, _warning.default)(false, 'Sender', `Failed to set end cursor: ${error}`);
      }
    }
  }, [focus, getRange, setRange]);
  const setStartCursor = (0, _react.useCallback)((targetNode, preventScroll) => {
    if (!targetNode) {
      return;
    }
    focus(targetNode, preventScroll);
    ...
    if (range && selection) {
      try {
        ...
        (0, _warning.default)(false, 'Sender', `Failed to set start cursor: ${error}`);
      }
    }
  }, [focus, getRange, setRange]);
  const setAllSelectCursor = (0, _react.useCallback)((targetNode, skillDom, preventScroll) => {
    if (!targetNode) {
      return;
    }
    focus(targetNode, preventScroll);
    ...
    if (range && selection) {
      try {
        ...
        (0, _warning.default)(false, 'Sender', `Failed to select all content: ${error}`);
      }
    }
  }, [focus, getRange, setRange]);
  const setCursorPosition = (0, _react.useCallback)((targetNode, editableNode, position, preventScroll) => {
    ...
    if (range && selection) {
      try {
        ...
        (0, _warning.default)(false, 'Sender', `Failed to set cursor position:: ${error}`);
      }
    }
    return {
      range: range,
  ...
  const setSlotFocus = (0, _react.useCallback)((editableRef, key, preventScroll = false) => {
  ...
  const setAfterNodeFocus = (0, _react.useCallback)((targetNode, editableNode, range, selection, preventScroll) => {
  ...
  const getTextBeforeCursor = (0, _react.useCallback)(targetNode => {
    // 快速路径：空节点直接返回
    if (!targetNode) {
      return {
        value: '',
    ...
    try {
      ...
      (0, _warning.default)(false, 'Sender', `Failed to get text before cursor: ${error}`);
      return {
        value: '',
        startContainer: null,
        startOffset: 0
   ...
   */
  const getInsertPosition = (0, _react.useCallback)((position, editableRef, lastSelectionRef) => {
   ...
   * 获取末尾插入范围
   */
  const getEndRange = (0, _react.useCallback)(editableDom => {
   ...
   * 获取开头插入范围
   */
  const getStartRange = (0, _react.useCallback)(editableDom => {
  ...
  const getCleanedText = (0, _react.useCallback)(ori => {
    return ori.replace(/\u200B/g, '') // 移除零宽空格
    .replace(/\n/g, '').replace(/^\n+|\n+$/g, ''); // 移除开头和结尾的换行
  }, []);
  const copySelectionString = (0, _react.useCallback)(async () => {
    try {
      const selection = getSelection();
      if (!selection) {
        return false;
      ...
      (0, _warning.default)(false, 'Sender', `Failed to copy selection: ${error}`);
      return false;
    }
...
var _default = exports.default = useCursor;
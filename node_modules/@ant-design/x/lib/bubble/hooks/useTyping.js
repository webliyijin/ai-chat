...
function useTyping({
  ...
  const animating = _react.default.useRef(false);
  const renderedData = _react.default.useRef('');
  const currentTask = _react.default.useRef(1);
  const raf = _react.default.useRef(-1);
  // 正在执行的 excuteAnimation 处于闭包内，但需要获取最新的 streaming。
  const streamingRef = _react.default.useRef(streaming);
  streamingRef.current = streaming;

  // typing legal check
  const memoedAnimationCfg = _react.default.useMemo(() => {
    const baseCfg = {
      effect: 'fade-in',
      interval: 100,
      step: 6,
  ...
  const typingSourceRef = _react.default.useRef({
    content,
  ...
  const excuteAnimation = _react.default.useCallback(taskId => {

...
function checkThresholds(results) {
  ...
  results.forEach(result => {
    ...
    if (duration > thresholds.maxDuration) {
      failures.push(`❌ ${textType} text: Duration ${duration.toFixed(0)}ms exceeds threshold ${thresholds.maxDuration}ms`);
    } else if (duration > thresholds.maxDuration * 0.9) {
      warnings.push(`⚠️  ${textType} text: Duration ${duration.toFixed(0)}ms is close to threshold ${thresholds.maxDuration}ms`);
    }
    if (avgFPS < thresholds.minAvgFPS) {
      failures.push(`❌ ${textType} text: Avg FPS ${avgFPS.toFixed(1)} is below threshold ${thresholds.minAvgFPS}`);
    } else if (avgFPS < thresholds.minAvgFPS * 1.1) {
      warnings.push(`⚠️  ${textType} text: Avg FPS ${avgFPS.toFixed(1)} is close to threshold ${thresholds.minAvgFPS}`);
    }
    if (stdDevFPS > thresholds.maxStdDevFPS) {
      failures.push(`❌ ${textType} text: FPS StdDev ${stdDevFPS.toFixed(2)} exceeds threshold ${thresholds.maxStdDevFPS}`);
    }
    const memoryDeltaMB = memoryDelta / 1024 / 1024;
    if (memoryDeltaMB > thresholds.maxMemoryDelta) {
      failures.push(`❌ ${textType} text: Memory delta ${memoryDeltaMB.toFixed(2)}MB exceeds threshold ${thresholds.maxMemoryDelta}MB`);
    } else if (memoryDeltaMB > thresholds.maxMemoryDelta * 0.9) {
      warnings.push(`⚠️  ${textType} text: Memory delta ${memoryDeltaMB.toFixed(2)}MB is close to threshold ${thresholds.maxMemoryDelta}MB`);
    }
  });
  return {
    failures,
...
function generateReport(currentResults) {
  const {
    failures,
    warnings
  } = checkThresholds(currentResults);
